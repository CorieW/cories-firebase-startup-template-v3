...
- `theme.ts`: Fixed admin document theme constants for dark-mode shell.
- `ui.ts`: Shared admin util class strings for buttons, inputs, badges, and layout.

## Writing Rules

- Keep shared helper modules small, dep-light, and split by concern so routes stay easy to review.
- Update file when immediate child files or the `server/` dir inventory changes.
