...
- `load-docs-page.ts`: Server fn wrapper for loading docs page from content source.
- `source.server.ts`: Server-only docs content source and search helpers.

## Writing Rules

- Keep server-only source access behind `*.server.ts` or server-fn boundaries so client bundles stay clean.
