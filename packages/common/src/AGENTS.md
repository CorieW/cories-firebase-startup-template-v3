...
- `client-api/`: Client-facing API surface exported by common package.
...
- `utils/`: Portable utils shared across packages.
...
- Export new public shared modules intentionally through `index.ts` or relevant child barrel; otherwise keep them internal to package.
- Update public barrel and downstream call sites together when shared contracts move.
