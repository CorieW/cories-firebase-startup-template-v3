...
- `auth-ui-toast.ts`: Dashboard compatibility wrapper that re-exports shared Better Auth toast helpers.
...
- `billing-api.ts`: Shared billing scope types used by dashboard billing UI.
...
- `route-guards.ts`: Route guard utils.
...
- Keep shared app logic here when it is not tied to single route or component subtree.
...
- Preserve stable helper APIs because many routes and components depend on dir.
